-- Optional targets preserve unknown legacy data; zero means explicitly none.
alter table public.clients
  add column ugc_creators_per_month integer check (ugc_creators_per_month >= 0),
  add column seeding_creators_per_month integer check (seeding_creators_per_month >= 0);
...
comment on column public.clients.ugc_creators_per_month is
  'Monthly UGC creator sourcing target. NULL is not yet recorded; zero means none required.';
comment on column public.clients.seeding_creators_per_month is
  'Monthly product-seeding creator sourcing target. NULL is not yet recorded; zero means none required.';
comment on column public.design_cs_allocations.package_type is
  'Creative package at month close. Historical rows remain NULL when not recorded.';
comment on column public.design_cs_allocations.ugc_creators_per_month is
  'Monthly UGC creator sourcing target frozen when this allocation month closes.';
comment on column public.design_cs_allocations.seeding_creators_per_month is
  'Monthly seeding creator sourcing target frozen when this allocation month closes.';
