-- Client operations access follows department membership so new Operations
-- roles do not need bespoke application and RLS changes. CEO access remains
-- intact, while CEO-only Design/CS import data and Hiring Roadmap stay private.

drop policy if exists "Operations department can manage clients" on public.clients;
create policy "Operations department can manage clients"
  ...
  using (exists (
    ...
    where profile.id = (select auth.uid())
      ...
      and lower(trim(coalesce(profile.department, ''))) = 'operations'
  ))
  with check (exists (
    select 1
    from public.profiles profile
...
create policy "Operations department can manage client health"
...
create policy "Operations department can manage client actions"
...
drop policy if exists "CEO and Ops can manage Design CS months" on public.design_cs_months;
...
drop policy if exists "CEO and Ops can manage Design CS people" on public.design_cs_people;
...
drop policy if exists "CEO and Ops can manage Design CS allocations" on public.design_cs_allocations;
...
drop policy if exists "CEO and Ops can manage Design CS settings" on public.design_cs_capacity_settings;
