  ...
  async function loadData() {
    ...
    try {
      ...
      try {
        ...
        // CS sees clients where their ID is in cs_ids array
      // MB sees clients where their ID is in mb_ids array
      const relevantClients = profile.position==='creative_strategist'
          ? (allClients||[]).filter(c=>(c.cs_ids||[]).includes(profile.id)||(c.assigned_cs_id===profile.id))
          : profile.position==='media_buyer'
          ? (allClients||[]).filter(c=>(c.mb_ids||[]).includes(profile.id))
          : (allClients||[])
