...
export default function Dashboard() {
  ...
  async function loadData() {
    ...
    // My OKRs — objectives for my role
    ...
    // Milestones for my role
    const msQ = supabase.from('milestones').select('*').eq('is_active',true).neq('status','completed')
    const {data:msData} = profile.position && !isManagement
      ? await msQ.eq('role_type',profile.position).limit(5)
      : await msQ.limit(5)
    ...
    // Spend — for CS: their assigned clients; for management: all clients
    const {data:allClients}=await supabase.from('clients').select('*').eq('is_active',true).order('name')
    const relevantClients = profile.position==='creative_strategist'
      ? (allClients||[]).filter(c=>c.assigned_cs_id===profile.id)
      : (allClients||[])
    ...
    if (relevantClients.length) {
      ...
      const totalDDU=(spendData||[]).reduce((s,e)=>s+(e.ddu_spend||0),0)
      const totalAll=(spendData||[]).reduce((s,e)=>s+(e.total_spend||0),0)
      ...
      setSpendSummary({totalDDU,totalAll,avgPct,atRisk,logged:spendData?.length||0,total:relevantClients.length})
    }
      ...
      <div className="page-body">
        ...
        <div style={{display:'grid',gridTemplateColumns:'1.2fr 1fr',gap:20}}>
          <div>
            ...
            {myKRs.length===0?(
              <div className="card mb-4 empty-state" style={{padding:'20px'}}><p>No key results assigned yet.</p></div>
            ):(
                ...
                {myKRs.map(kr=>{
                  ...
                  return (
                    <div key={kr.id} className={`kpi-card ${status}`}>
                      <div style={{fontSize:10,color:'var(--text-muted)',fontFamily:'var(--font-mono)',marginBottom:4}}>{kr.objective_title}</div>
            ...
            {/* Spend Tracker Summary — show for CS and management */}
            {myClients.length>0&&(
              <>
                <div className="section-header">
                  <span className="section-title">
                    {profile.position==='creative_strategist'?'My Client Spend':'Spend — This Week'}
                  </span>
                ...
                <div className="card mb-3" style={{borderLeft:`3px solid ${spendStatus?.color||'var(--border)'}`,padding:'12px 16px'}}>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:spendSummary.avgPct?10:0}}>
                    <div><div className="card-label">DDU Spend</div><div style={{fontFamily:'var(--font-display)',fontSize:18,fontWeight:700,letterSpacing:'-0.03em',color:'var(--accent)'}}>{fmtMoney(spendSummary.totalDDU)}</div></div>
                    <div><div className="card-label">Total Spend</div><div style={{fontFamily:'var(--font-display)',fontSize:18,fontWeight:700,letterSpacing:'-0.03em'}}>{fmtMoney(spendSummary.totalAll)}</div></div>
                    <div><div className="card-label">Avg DDU %</div><div style={{fontFamily:'var(--font-display)',fontSize:18,fontWeight:700,letterSpacing:'-0.03em',color:spendStatus?.color||'var(--text-muted)'}}>{spendSummary.avgPct!==null?`${spendSummary.avgPct}%`:'—'}</div></div>
                  </div>
                  {spendSummary.avgPct&&(
                ...
                <div className="card" style={{padding:0,overflow:'hidden'}}>
                  {myClients.slice(0,8).map((client,i)=>{
                    ...
                    return (
                      <div key={client.id} style={{display:'flex',alignItems:'center',gap:12,padding:'9px 14px',borderBottom:i<myClients.slice(0,8).length-1?'1px solid var(--border)':'none'}}>
                          ...
                          <>
                            <div style={{flex:1,minWidth:60}}>
                              <div style={{height:4,background:'var(--border)',borderRadius:2,overflow:'hidden',position:'relative'}}>
                                ...
                                <div style={{width:`${Math.min(100,pct||0)}%`,height:'100%',background:s?.color,borderRadius:2}}/>
                              </div>
                            </div>
