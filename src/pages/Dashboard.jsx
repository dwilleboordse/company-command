...
export default function Dashboard() {
  ...
  async function loadData() {
    ...
    try {
      ...
      // Spend
      try {
        const {data:allClients}=await supabase.from('clients').select('id,name,cs_ids,mb_ids,editor_ids,designer_ids,ugc_ids,assigned_cs_id').eq('is_active',true).eq('is_archived',false).order('name')
        // Filter clients by assignment — check all role arrays
