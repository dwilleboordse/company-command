    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="grid-2">
          <div className="form-group"><label>Department</label><select value={form.department} onChange={e=>setForm({...form,department:e.target.value})}>{['marketing','delivery','operations','management'].map(d=><option key={d} value={d}>{d}</option>)}</select></div>
          <div className="form-group"><label>Role Type</label><select value={form.role_type} onChange={e=>setForm({...form,role_type:e.target.value})}>{Object.entries(ROLE_LABELS).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></div>
        </div>
...
function InlineEdit({value,onSave,onCancel}) {
  ...
  return (
    <div className="flex items-center gap-2" style={{flex:1}}>
      <input value={v} onChange={e=>setV(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')onSave(v);if(e.key==='Escape')onCancel()}} style={{fontSize:13,padding:'4px 8px'}} autoFocus/>
...
function SystemGroup({systemName,milestones,dept,role,canEdit,onStatusChange,onDelete,onDeleteSystem,onRename,onRenameSystem}) {
  const [open,setOpen]=useState(false)
  const [editingSystem,setEditingSystem]=useState(false)
  const [editingId,setEditingId]=useState(null)
  const p=pct(milestones)
  const allDone=milestones.every(m=>m.status==='completed')

  async function deleteSystem() {
    if (!confirm(`Delete entire system "${systemName}" and all its milestones?`)) return
    onDeleteSystem(dept, role, systemName)
  }
    ...
    <div className="ms-system">
      <div className="ms-system-header" onClick={()=>!editingSystem&&setOpen(!open)}>
        <div className="flex items-center gap-3" style={{flex:1}}>
          <div style={{width:32,height:32,borderRadius:'50%',background:allDone?'var(--green-dim)':'var(--bg)',border:`2px solid ${allDone?'var(--green)':'var(--border)'}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontFamily:'var(--font-mono)',fontWeight:700,color:allDone?'var(--green)':'var(--text-muted)',flexShrink:0}}>{p}%</div>
        ...
        <div className="flex items-center gap-2">
          {canEdit&&!editingSystem&&(
            <>
              <button className="btn btn-ghost btn-icon btn-sm" onClick={e=>{e.stopPropagation();setEditingSystem(true)}} title="Rename system"><Edit2 size={12}/></button>
              <button className="btn btn-danger btn-icon btn-sm" onClick={e=>{e.stopPropagation();deleteSystem()}} title="Delete system"><Trash2 size={12}/></button>
            </>
          )}
          <div className="progress-bar-wrap" style={{width:80,marginTop:0}}><div className="progress-bar-fill green" style={{width:`${p}%`}}/></div>
        ...
        <div className="ms-system-body">
          {milestones.map(ms=>(
            <div key={ms.id} className="ms-item">
              ...
              {canEdit&&editingId!==ms.id&&<button className="btn btn-ghost btn-icon btn-sm" onClick={()=>setEditingId(ms.id)}><Edit2 size={12}/></button>}
              {canEdit&&<button className="btn btn-danger btn-icon btn-sm" onClick={()=>onDelete(ms.id)}><Trash2 size={12}/></button>}
            </div>
          ))}
        </div>
      )}
...
export default function Milestones() {
  ...
  useEffect(()=>{load()},[profile,deptFilter,statusFilter,memberFilter])
  ...
  async function handleDelete(id) {
    if (!confirm('Delete this milestone step?')) return
    await supabase.from('milestones').update({is_active:false}).eq('id',id)
    setMilestones(prev=>prev.filter(m=>m.id!==id))
  }
  async function handleDeleteSystem(dept,role,sysName) {
    const ids=milestones.filter(m=>m.department===dept&&m.role_type===role&&m.system_name===sysName).map(m=>m.id)
    for (const id of ids) await supabase.from('milestones').update({is_active:false}).eq('id',id)
    setMilestones(prev=>prev.filter(m=>!(m.department===dept&&m.role_type===role&&m.system_name===sysName)))
  }
  ...
  const grouped=milestones.reduce((acc,ms)=>{
    ...
    acc[dk][rk][sk].push(ms);return acc
  },{})

  const overallPct=pct(milestones)
    ...
    <>
      ...
      <div className="page-body">
        <div className="stat-row">
          <div className="stat-box"><div className="stat-box-label">Overall</div><div className="stat-box-value text-accent">{overallPct}%</div></div>
        ...
        <div className="flex gap-3 mb-6" style={{flexWrap:'wrap'}}>
          ...
          {isManagement&&(
            <select value={memberFilter} onChange={e=>setMemberFilter(e.target.value)} style={{width:'auto'}}>
              ...
              {Object.entries(roles).map(([role,systems])=>(
                <div key={role} style={{marginBottom:24}}>
                  ...
                  {Object.entries(systems).map(([sysName,items])=>(
                    <SystemGroup key={sysName} systemName={sysName} milestones={items} dept={dept} role={role}
      ...
      {showAdd&&<AddMilestoneModal onClose={()=>setShowAdd(false)} onSave={load}/>}
    </>
  )
}
