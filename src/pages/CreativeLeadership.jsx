...
import { formatSpendMoney, isCompleteSpendEntry, spendShare, summarizeSpend } from '../lib/spendAnalytics'
...
function CoachingForm({ note, data, busy, onDirty, onCancel, onSave }) {
  const initial = { strategist_id: note.strategist_id || '', client_id: note.client_id || '', observation: note.observation || '', expected_standard: note.expected_standard || '', agreed_action: note.agreed_action || '', due_date: note.due_date || '', follow_up: note.follow_up || '', outcome: note.outcome || 'open' }
  const [form, setForm] = useState(initial)
  const [error, setError] = useState('')
