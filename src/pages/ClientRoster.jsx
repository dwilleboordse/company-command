    ...
    <div className="card" style={{padding:0,overflow:'hidden',display:'flex',flexDirection:'column'}}>
      ...
      {totalAds>0&&(
        <div style={{padding:'8px 16px',borderBottom:'1px solid var(--border)'}}>
          <div style={{fontSize:9,fontFamily:'var(--font-mono)',color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:1,fontWeight:600,marginBottom:6}}>Monthly Package</div>
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            {totalVideo>0&&(
              <div style={{textAlign:'center',padding:'4px 10px',borderRadius:'var(--radius)',background:'var(--bg)',border:'1px solid var(--border)'}}>
                <div style={{fontSize:14,fontWeight:700,color:'var(--accent)'}}>{totalVideo}</div>
                <div style={{fontSize:9,color:'var(--text-muted)',fontFamily:'var(--font-mono)',textTransform:'uppercase'}}>
                  ...
                  <span style={{display:'block',color:'var(--text-muted)',fontWeight:400}}>
                    {creatives.video?.concepts||0}c × {creatives.video?.variations||0}v
                  </span>
                </div>
              </div>
            )}
            {totalUgc>0&&(
              <div style={{textAlign:'center',padding:'4px 10px',borderRadius:'var(--radius)',background:'var(--bg)',border:'1px solid var(--border)'}}>
                <div style={{fontSize:14,fontWeight:700,color:'#f59e0b'}}>{totalUgc}</div>
                <div style={{fontSize:9,color:'var(--text-muted)',fontFamily:'var(--font-mono)',textTransform:'uppercase'}}>
                  UGC
                  <span style={{display:'block',color:'var(--text-muted)',fontWeight:400}}>
                    {creatives.ugc?.concepts||0}c × {creatives.ugc?.variations||0}v
                  </span>
                </div>
              </div>
            )}
            {totalStatic>0&&(
              <div style={{textAlign:'center',padding:'4px 10px',borderRadius:'var(--radius)',background:'var(--bg)',border:'1px solid var(--border)'}}>
                <div style={{fontSize:14,fontWeight:700,color:'#8b5cf6'}}>{totalStatic}</div>
                <div style={{fontSize:9,color:'var(--text-muted)',fontFamily:'var(--font-mono)',textTransform:'uppercase'}}>
                  Statics
                  <span style={{display:'block',color:'var(--text-muted)',fontWeight:400}}>
                    {creatives.static?.concepts||0}c × {creatives.static?.variations||0}v
                  </span>
                </div>
              </div>
            )}
            <div style={{marginLeft:'auto',textAlign:'right',display:'flex',flexDirection:'column',justifyContent:'center'}}>
              <div style={{fontSize:18,fontWeight:800,color:'var(--text-primary)'}}>{totalAds}</div>
              <div style={{fontSize:9,color:'var(--text-muted)',fontFamily:'var(--font-mono)',textTransform:'uppercase'}}>total/mo</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="page-body">
        ...
        <div className="tabs" style={{marginBottom:0}}>
          ...
          <button className={`tab ${activeTab==='archived'?'active':''}`} onClick={()=>setActiveTab('archived')}>
            <Archive size={12} style={{marginRight:4}}/> Archived ({archivedClients.length})
          </button>
        </div>

        {/* Search + View Toggle */}
        <div className="flex items-center gap-3 mb-4" style={{flexWrap:'wrap',marginTop:16}}>
        ...
        {loading ? (
          ...
          archivedClients.length===0
            ? <div className="empty-state"><p>No archived clients.</p></div>
            : <div className="card" style={{padding:0,overflow:'hidden'}}>
                {archivedClients.filter(c=>!search||c.name.toLowerCase().includes(search.toLowerCase())).map((c,i,arr)=>(
                  <div key={c.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',
                    padding:'12px 16px',borderBottom:i<arr.length-1?'1px solid var(--border)':'none'}}>
                    <div style={{display:'flex',alignItems:'center',gap:10}}>
                      <div style={{width:32,height:32,borderRadius:'var(--radius)',background:'var(--bg)',
                        border:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'center',
                        fontSize:11,fontWeight:700,color:'var(--text-muted)'}}>
                        {c.name?.slice(0,2).toUpperCase()}
                      </div>
                      <div>
                        <div style={{fontSize:13,fontWeight:600,color:'var(--text-secondary)'}}>{c.name}</div>
                        {c.package_type&&<div style={{fontSize:10,color:'var(--text-muted)',fontFamily:'var(--font-mono)'}}>{c.package_type}</div>}
                      </div>
                    </div>
