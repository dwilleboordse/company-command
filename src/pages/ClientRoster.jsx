...
function ClientModal({ client, allMembers, onClose, onSave }) {
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      cs_ids: form.cs_ids, mb_ids: form.mb_ids,
      editor_ids: form.editor_ids, designer_ids: form.designer_ids, ugc_ids: form.ugc_ids,
      assigned_cs_id: form.cs_ids[0]||null,
      channels: form.channels, creatives: form.creatives,
    ...
    if (isEdit) {
      ...
      // Auto-create client health entry
      if (newClient?.id) {
        const weekStart = new Date()
        weekStart.setDate(weekStart.getDate()-weekStart.getDay()+1)
        const ws = weekStart.toISOString().split('T')[0]
        await supabase.from('client_health_entries').upsert({client_id:newClient.id,week_start:ws,performance_health:0,creative_strategy:0,execution_delivery:0,strategic_alignment:0,communication:0},{onConflict:'client_id,week_start'})
      }
    }
