...
function InitiativeRow({ ms, kr, allMembers, isManagement, onEdit, onDelete, onStatusChange }) {
  ...
  return (
    <div style={{display:'flex',alignItems:'center',gap:12,padding:'9px 0',borderBottom:'1px solid var(--border)'}}>
      {/* Status color dot */}
      <div style={{width:8,height:8,borderRadius:'50%',background:sInfo.color,flexShrink:0}}/>

      {/* Name + notes — takes all available space */}
      <div style={{flex:'1 1 0',width:0}}>
        <div style={{fontSize:13,fontWeight:isDone?400:500,
      ...
      {/* Status dropdown — only way to change status */}
      <select value={ms.status} onClick={e=>e.stopPropagation()}
        ...
        style={{fontSize:11,padding:'4px 10px',borderRadius:'var(--radius)',
          ...
          color:sInfo.color,background:sInfo.bg||'var(--bg)',
          border:`1.5px solid ${sInfo.color}40`,cursor:'pointer',flexShrink:0}}>
