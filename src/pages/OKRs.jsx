...
function ObjectiveModal({ existing, allMembers, onClose, onSave }) {
  ...
  const [form, setForm] = useState({
    ...
    quarter:existing?.quarter||quarter, assignee_ids:existing?.assignee_ids||[],
  })
  const [saving, setSaving] = useState(false)
  ...
  async function handleSave() {
    ...
    else await supabase.from('objectives').insert({...form,status:'active',assignee_ids:form.assignee_ids||[]})
    onSave(); setSaving(false); onClose()
  }
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{maxWidth:520}} onClick={e=>e.stopPropagation()}>
        ...
        {allMembers?.length>0&&(
          <div className="form-group">
            <label>Assigned Team Members (multi-select)</label>
            <div style={{display:'flex',flexWrap:'wrap',gap:6,marginTop:4}}>
              {allMembers.map(m=>{
                const sel=(form.assignee_ids||[]).includes(m.id)
                return (
                  <div key={m.id} onClick={()=>setForm({...form,assignee_ids:sel?(form.assignee_ids||[]).filter(x=>x!==m.id):[...(form.assignee_ids||[]),m.id]})}
                    style={{display:'flex',alignItems:'center',gap:6,padding:'5px 10px',borderRadius:100,
                      border:`2px solid ${sel?'var(--accent)':'var(--border)'}`,
                      background:sel?'var(--accent-dim)':'var(--bg-input)',cursor:'pointer',
                      fontSize:12,fontWeight:sel?600:400,color:sel?'var(--accent)':'var(--text-secondary)'}}>
                ...
                return (
                  <div key={m.id} onClick={()=>setForm({...form,assignee_ids:sel?(form.assignee_ids||[]).filter(x=>x!==m.id):[...(form.assignee_ids||[]),m.id]})}
                    style={{display:'flex',alignItems:'center',gap:6,padding:'5px 10px',borderRadius:100,
                      ...
                      background:sel?'var(--green-dim)':'var(--bg-input)',cursor:'pointer',
                      fontSize:12,fontWeight:sel?600:400,color:sel?'var(--green)':'var(--text-secondary)'}}>
                    <div className="user-avatar" style={{width:20,height:20,fontSize:8}}>
                      {m.full_name?.split(' ').map(n=>n[0]).join('').slice(0,2)}
                    </div>
                    {m.full_name?.split(' ')[0]}
...
function InitiativeModal({ kr, existing, allMembers, objectiveRoleType, onClose, onSave }) {
  const ROLE_LABELS_LOCAL = {
    ...
    ops_manager:'Operations Manager', ops_assistant:'Operations Assistant', hr_manager:'HR Manager', management:'Management',
  }
  ...
  const [form, setForm] = useState({
    ...
    milestone_name: existing?.milestone_name || '',
    department: existing?.department || 'delivery',
    role_type: existing?.role_type || objectiveRoleType || 'creative_strategist',
  ...
  async function loadUnlinked() {
    const { data } = await supabase.from('milestones').select('*')
      .eq('is_active', true).is('key_result_id', null)
      .neq('status', 'completed').order('system_name')
    setExistingMilestones(data || [])
  }

  function toggleAssignee(id) {
    const ids = form.assignee_ids || []
    setForm({...form, assignee_ids: ids.includes(id) ? ids.filter(x=>x!==id) : [...ids, id]})
  }
  ...
  async function handleSave() {
    ...
    if (existing) {
      ...
      await supabase.from('milestones').insert({
        ...form, status: form.status, key_result_id: kr.id,
        is_active: true
      })
    }
    onSave(); setSaving(false); onClose()
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 520 }} onClick={e => e.stopPropagation()}>
        <h2 className="modal-title" style={{ marginBottom: 4 }}>{isEdit ? 'Edit Initiative' : 'Add Initiative'}</h2>
        {!isEdit && <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 16 }}>
          Supporting KR: <strong style={{ color: 'var(--amber)' }}>{kr.metric_name}</strong>
        </p>}
          ...
          <>
            <div className="form-group">
              ...
              <input value={form.system_name} onChange={e => setForm({...form, system_name: e.target.value})}
                placeholder="e.g. Approval Rate Optimization" autoFocus={!isEdit}/>
            </div>
            <div className="form-group">
              <label>Initiative / Action Step</label>
              <input value={form.milestone_name} onChange={e => setForm({...form, milestone_name: e.target.value})}
                placeholder="e.g. Build standardized brief template"/>
            </div>
            <div className="grid-2">
              <div className="form-group">
            ...
            <div className="flex gap-2 mt-2">
              <button className="btn btn-primary" onClick={handleSave}
                disabled={saving || !form.system_name || !form.milestone_name}>
        ...
        {tab === 'link' && !isEdit && (
          <>
            ...
            {existingMilestones.length === 0 ? (
              <div className="empty-state" style={{ padding: '20px 0' }}><p>No unlinked milestones found.</p></div>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:6, maxHeight:300, overflowY:'auto' }}>
                {existingMilestones.map(ms => (
                  <div key={ms.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px',
                    background:'var(--bg-input)', border:'1px solid var(--border)', borderRadius:'var(--radius)', cursor:'pointer' }}
                    onClick={() => handleLink(ms.id)}>
                    <div className={`ms-dot ${ms.status}`}/>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, fontWeight:500 }}>{ms.milestone_name}</div>
...
// ── KR ROW ───────────────────────────────────────────────────
function KRRow({ kr, members, krValues, milestones, onLog, onEdit, onDelete, onInitiativeChange, isManagement, isCEO }) {
  ...
  return (
    <div style={{border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',overflow:'hidden',marginBottom:8}}>
        ...
        <div style={{background:'var(--bg)',borderTop:'1px solid var(--border)'}}>
          ...
          <div style={{padding:'12px 14px',borderTop:'1px solid var(--border)'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
              <div style={{fontSize:10,fontFamily:'var(--font-mono)',color:'var(--amber)',textTransform:'uppercase',letterSpacing:1}}>
                Initiatives {initiatives.length>0&&`· ${doneInit}/${initiatives.length} done`}
              </div>
              {isManagement&&(
                <button className="btn btn-ghost btn-sm" style={{fontSize:11}}
                  onClick={e=>{e.stopPropagation();setAddingInitiative(true)}}>
            ...
            {initiatives.length===0?(
              <p style={{fontSize:11,color:'var(--text-muted)'}}>
                No initiatives yet.{isManagement?' Add one to define the actions that drive this KR.':' Check back when management links actions here.'}
              </p>
            ):initiatives.map(ms=>{
              const assigneeList = (ms.assignee_ids||[])
              ...
              return (
                <div key={ms.id} style={{padding:'8px 0',borderBottom:'1px solid var(--border)'}}>
                    ...
                    {isManagement&&(
                      <div className="flex gap-1" onClick={e=>e.stopPropagation()}>
                        <button className="btn btn-ghost btn-icon btn-sm"
                          onClick={()=>setEditingInitiative(ms)}><Edit2 size={11}/></button>
                  ...
                  {assigneeList.length>0&&members.length>0&&(
                    <div style={{display:'flex',gap:4,marginTop:5,marginLeft:22}}>
                      {assigneeList.map(id=>{
                        const m=members.find(x=>x.id===id)
                        if (!m) return null
                        return (
                          <div key={id} title={m.full_name} className="user-avatar"
                            style={{width:20,height:20,fontSize:8,border:'1px solid var(--border)'}}>
                            {m.full_name?.split(' ').map(n=>n[0]).join('').slice(0,2)}
                          </div>
                        )
                      })}
      ...
      {addingInitiative&&(
        <InitiativeModal kr={kr} allMembers={members} objectiveRoleType={null}
          onClose={()=>setAddingInitiative(false)}
          onSave={()=>{setAddingInitiative(false);onInitiativeChange&&onInitiativeChange()}}
        />
      )}
      {editingInitiative&&(
        <InitiativeModal kr={kr} existing={editingInitiative} allMembers={members} objectiveRoleType={null}
...
// ── OBJECTIVE CARD ────────────────────────────────────────────
function ObjectiveCard({ objective, keyResults, members, krValues, milestones, onAddKR, onEditKR, onDeleteKR, onEditObj, onDeleteObj, onLog, onInitiativeChange, isManagement, isCEO, allMembers }) {
        ...
        <div className="flex items-center justify-between" style={{flexWrap:'wrap',gap:10}}>
          <div style={{flex:1}}>
            <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',marginBottom:6}}>
              ...
              {/* Assignees */}
              {(objective.assignee_ids||[]).length>0&&(
                <div style={{display:'flex',gap:3}}>
                  {(objective.assignee_ids||[]).map(id=>{
                    const m=members.find(x=>x.id===id)
                    if (!m) return null
                    return <div key={id} title={m.full_name} className="user-avatar" style={{width:18,height:18,fontSize:7,border:`1px solid ${color}`}}>{m.full_name?.split(' ').map(n=>n[0]).join('').slice(0,2)}</div>
                  })}
                </div>
              )}
            </div>
        ...
        <div style={{padding:'16px 18px',borderTop:'1px solid var(--border)',background:'var(--bg)'}}>
          ...
          {visibleKRs.length===0?(
            ...
            <KRRow key={kr.id} kr={kr} members={members} krValues={krValues}
              ...
              onInitiativeChange={onInitiativeChange} isManagement={isManagement} isCEO={isCEO}/>
          ))}
        </div>
      )}
    </div>
...
export default function OKRs() {
  ...
  async function load() {
    ...
    if (objData?.length) {
      ...
      // Load initiatives (milestones linked to KRs)
      if (krData?.length) {
  ...
  return (
    <>
      ...
      {showAddObj&&<ObjectiveModal allMembers={members} onClose={()=>setShowAddObj(false)} onSave={load}/>}
      {editObj&&<ObjectiveModal existing={editObj} allMembers={members} onClose={()=>setEditObj(null)} onSave={load}/>}
      {addKRFor&&<KRModal objectiveId={addKRFor} allMembers={members} onClose={()=>setAddKRFor(null)} onSave={load}/>}
      {editKR&&<KRModal existing={editKR} objectiveId={editKR.objective_id} allMembers={members} onClose={()=>setEditKR(null)} onSave={load}/>}
