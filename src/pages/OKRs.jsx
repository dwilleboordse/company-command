...
function KRRow({ kr, members, krValues, milestones, allMembers, onLog, onEdit, onDelete, onInitiativeChange, isManagement, isCEO, profileId }) {
  ...
  // Only show members assigned to THIS KR. If KR has no assignees, fall back to objective-level members.
  const krDisplayMembers = krAssignees.length > 0
    ? members.filter(m => krAssignees.includes(m.id))
    : members
    ...
    <div style={{border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',overflow:'hidden',marginBottom:8,boxShadow:'var(--shadow-xs)'}}>
        ...
        <div className="flex items-center justify-between" style={{flexWrap:'wrap',gap:8}}>
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:10,color:'var(--text-muted)',fontFamily:'var(--font-mono)',marginTop:3,marginLeft:34}}>
              ...
              {krDisplayMembers.length>0&&<span style={{marginLeft:10}}>
                {memberLatestVals.length}/{krDisplayMembers.length} logged
      ...
      {expanded&&(
        <div style={{background:'var(--bg)',borderTop:'1px solid var(--border)'}}>
          {/* Member rows — only assignees of this KR */}
          <div style={{borderBottom:'1px solid var(--border)'}}>
            {krDisplayMembers.map(m=>{
