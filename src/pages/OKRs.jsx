...
function AddInitiativeModal({ kr, objectiveRoleType, onClose, onSave }) {
  ...
  const [tab, setTab] = useState('new') // 'new' or 'link'
  const [form, setForm] = useState({
    system_name: '', milestone_name: '',
    department: 'delivery', role_type: objectiveRoleType || 'creative_strategist',
  })
  const [existingMilestones, setExistingMilestones] = useState([])
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    if (tab === 'link') loadUnlinked()
  }, [tab])
  ...
  async function handleCreate() {
    ...
    await supabase.from('milestones').insert({
      ...form, status: 'not_started', key_result_id: kr.id
    })
    onSave(); setSaving(false); onClose()
  }
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 520 }} onClick={e => e.stopPropagation()}>
        <h2 className="modal-title" style={{ marginBottom: 4 }}>Add Initiative</h2>
        <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 16 }}>
          Supporting KR: <strong style={{ color: 'var(--amber)' }}>{kr.metric_name}</strong>
        </p>

        <div className="card mb-4" style={{ padding: '10px 14px', borderLeft: '3px solid var(--amber)', background: 'rgba(245,158,11,0.04)' }}>
          <p style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
            <strong style={{ color: 'var(--amber)' }}>Initiative</strong> = A specific action or project that directly drives progress on this Key Result. These are your milestones — the concrete steps your team takes.
          </p>
        </div>

        <div className="tabs" style={{ marginBottom: 16 }}>
          <button className={`tab ${tab === 'new' ? 'active' : ''}`} onClick={() => setTab('new')}>Create New</button>
          <button className={`tab ${tab === 'link' ? 'active' : ''}`} onClick={() => setTab('link')}>Link Existing Milestone</button>
        </div>
          ...
          <>
            <div className="form-group">
              ...
              <input value={form.system_name} onChange={e => setForm({ ...form, system_name: e.target.value })}
                placeholder="e.g. Approval Rate Optimization System" autoFocus />
            </div>
            <div className="form-group">
            ...
            <div className="flex gap-2 mt-2">
              <button className="btn btn-primary" onClick={handleCreate} disabled={saving || !form.system_name || !form.milestone_name}>
        ...
        {tab === 'link' && (
          <>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
              Select an existing milestone to link as an initiative for this KR. Only unlinked milestones are shown.
            </p>
            {existingMilestones.length === 0 ? (
              <div className="empty-state" style={{ padding: '20px 0' }}>
                <p>No unlinked milestones found. Create a new initiative instead.</p>
              </div>
            ) : (
                  ...
                  <div key={ms.id} style={{
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
                        {ms.system_name} · {ms.role_type}
                      </div>
                    </div>
                    <span className="btn btn-primary btn-sm" style={{ fontSize: 11 }}>Link</span>
                  </div>
                ))}
              </div>
            )}
    ...
    <div style={{border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',overflow:'hidden',marginBottom:8}}>
      ...
      {expanded&&(
        <div style={{background:'var(--bg)',borderTop:'1px solid var(--border)'}}>
          ...
          <div style={{padding:'12px 14px',borderTop:initiatives.length>0||isManagement?'1px solid var(--border)':'none'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:initiatives.length>0?10:0}}>
            ...
            {initiatives.length===0?(
              <p style={{fontSize:11,color:'var(--text-muted)'}}>
                No initiatives linked yet.{isManagement?' Click "Add Initiative" to create or link one.':' Initiatives will appear here when management links milestones to this KR.'}
              </p>
            ):initiatives.map(ms=>(
              <div key={ms.id} className="flex items-center gap-2" style={{padding:'7px 0',borderBottom:'1px solid var(--border)'}}>
                <div className={`ms-dot ${ms.status}`}/>
                <div style={{flex:1}}>
      ...
      {addingInitiative&&(
        <AddInitiativeModal kr={kr} objectiveRoleType={null}
...
// ── OBJECTIVE CARD ────────────────────────────────────────────
function ObjectiveCard({ objective, keyResults, members, krValues, milestones, onAddKR, onEditKR, onDeleteKR, onEditObj, onDeleteObj, onLog, onInitiativeChange, isManagement, isCEO }) {
