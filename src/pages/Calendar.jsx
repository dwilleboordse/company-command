...
import { today, getMonday, getMondayStr, addDays, parseLocal, fmtDisplay, DAYS, local } from '../lib/dates'
...
function WeekOutcomePanel({ userId, weekStart, isOwn }) {
  ...
  function removeOutcome(i) {
    const o = outcomes.filter((_, idx) => idx !== i)
    const od = outcomesDone.filter(x => x !== i).map(x => x > i ? x - 1 : x)
...
function MonthView({ userId, month, year }) {
  ...
  async function loadMonth() {
    ...
    const { data } = await supabase.from('day_entries').select('entry_date,day_outcome,day_outcome_done')
      .eq('user_id', userId)
      .gte('entry_date', local(firstDay))
      .lte('entry_date', local(lastDay))
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 1, background: 'var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
        ...
        {Array.from({ length: totalCells }, (_, i) => {
          ...
          if (dayNum < 1 || dayNum > lastDayOfMonth.getDate()) return <div key={i} style={{ background: 'var(--bg)', minHeight: 80 }} />
          const d = new Date(year, month, dayNum)
          const dateStr = local(d)
          ...
          return (
            <div key={i} style={{ background: isToday ? 'rgba(59,130,246,0.06)' : isWeekend ? 'rgba(255,255,255,0.01)' : 'var(--bg-card)', padding: 8, minHeight: 80, borderLeft: isToday ? '2px solid var(--accent)' : 'none' }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: isToday ? 'var(--accent)' : isWeekend ? 'var(--text-muted)' : 'var(--text-secondary)', marginBottom: 4 }}>{dayNum}</div>
              {!isWeekend && entry?.day_outcome && (
                <div style={{ fontSize: 10, color: entry.day_outcome_done ? 'var(--green)' : 'var(--text-secondary)', lineHeight: 1.3 }}>
...
export default function Calendar() {
  ...
  return (
    <>
      ...
      <div className="page-body" style={{ padding: '24px 20px' }}>
        {calView === 'month' ? (
          <MonthView userId={profile?.id} month={monthDate.getMonth()} year={monthDate.getFullYear()} />
        ) : (
          displayMembers.filter(Boolean).map(member => (
            <div key={member.id} style={{ marginBottom: 32, overflowX: 'auto' }}>
              {viewMode === 'team' && (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '160px repeat(5,1fr)', gap: 1, background: 'var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', border: '1px solid var(--border)', minWidth: 600 }}>
                ...
                {DAYS.map((day, i) => (
                  <div key={day} style={{ background: 'var(--bg)', padding: '10px 12px', borderLeft: '1px solid var(--border)' }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: local(weekDays[i]) === today() ? 'var(--accent)' : 'var(--text-primary)' }}>{day}</div>
                ...
                <div style={{ background: 'var(--bg-card)', padding: 12 }}>
                  <WeekOutcomePanel userId={member.id} weekStart={local(weekStart)} isOwn={member.id === profile?.id} />
                </div>
                {weekDays.map((day, i) => (
