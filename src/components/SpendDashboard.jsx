...
export default function SpendDashboard() {
  ...
  const visible = isCEO || profile?.position === 'creative_strategist'
  if (!visible) return null
  return <SpendDashboardContent profile={profile} isCEO={isCEO} />
}

function SpendDashboardContent({ profile, isCEO }) {
  ...
  const load = useCallback(async () => {
    try {
      const next = await fetchSpendData(profile, isCEO)
      setData(next)
      setError('')
  ...
  useEffect(() => {
    let cancelled = false
    fetchSpendData(profile, isCEO).then(next => { if (!cancelled) { setData(next); setError('') } })
  ...
  const displayedRows = isCEO && !showAllClients ? rows.slice(0, 8) : rows
  ...
  return <section className="card spend-section" aria-label={isCEO ? 'Business spend overview' : 'My client spend'}>
    <div className="spend-heading"><div><h2>{isCEO ? 'Business spend overview' : 'My client spend'}</h2><p className="spend-note" style={{ marginBottom: 0 }}>Week of {weekLabel(week)} · log the prior week every Monday</p></div><div className="spend-actions"><Link className="btn btn-ghost btn-sm" to="/spend?tab=analytics">Analytics <ArrowUpRight size={13} /></Link><Link className="btn btn-primary btn-sm" to="/spend">Open Spend Tracker <ArrowUpRight size={13} /></Link></div></div>
    {loading ? <div className="spend-empty">Loading spend…</div> : error ? <div role="alert" className="spend-error">{error} <button className="btn btn-ghost btn-sm" onClick={load}>Retry</button></div> : <>
      ...
      {isCEO && <div style={{ marginBottom: 22 }}>
      ...
      {!clients.length ? <p className="spend-empty">{isCEO ? 'No active clients in the roster.' : 'No active clients assigned to you in Client Roster.'}</p> : <>
        ...
        {isCEO && rows.length > 8 && <button className="btn btn-ghost btn-sm" style={{ marginTop: 10 }} onClick={() => setShowAllClients(value => !value)}>{showAllClients ? 'Show first 8 · missing and low share first' : `Show all ${rows.length} active clients`}</button>}
        <p className="spend-note">Active {isCEO ? 'company' : 'assigned'} clients only. Totals reflect saved entries; coverage may differ from the previous week. DDU share measures creative adoption, not revenue or ROAS.</p>
      </>}
    </>}
